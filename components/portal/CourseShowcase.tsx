    ...
    <section className="py-24 bg-white relative overflow-hidden">
      ...
      <div className="absolute top-0 left-0 w-full h-full pointer-events-none opacity-20">
        <motion.div 
          ...
          className="absolute top-10 right-10 grid grid-cols-5 gap-3"
        >
          {[...Array(25)].map((_, i) => (
      ...
      <div className="container mx-auto px-8 relative z-10">
          ...
          <div className="flex bg-slate-100 p-1.5 rounded-full mt-10 w-fit border border-slate-200">
            <button
              ...
              className={`px-8 py-2.5 rounded-full font-bold text-sm transition-all duration-300 relative ${
                activeTab === 'zica' ? 'text-white' : 'text-slate-500 hover:text-slate-800'
              }`}
            >
              {activeTab === 'zica' && (
                <motion.div 
            ...
            <button
              ...
              className={`px-8 py-2.5 rounded-full font-bold text-sm transition-all duration-300 relative ${
                activeTab === 'zima' ? 'text-white' : 'text-slate-500 hover:text-slate-800'
              }`}
            >
              {activeTab === 'zima' && (
                <motion.div 
        ...
        <div className="flex flex-col items-center">
          <Link 
            ...
            className="group flex items-center gap-3 text-slate-900 font-extrabold text-lg hover:text-primary transition-colors"
          >
            <span>Explore more {activeTab.toUpperCase()} courses</span>
            <div className="w-10 h-10 rounded-full border-2 border-slate-200 flex items-center justify-center group-hover:border-primary group-hover:bg-primary group-hover:text-white transition-all">
